...
{	
	...
	recBuffer = (unsigned char*) malloc((size_t)LinxDev->ListenerBufferSize);
	sendBuffer = (unsigned char*) malloc((size_t)LinxDev->ListenerBufferSize);
...
int LinxSerialListener::Connected()
{
	...
	else
	{
		...
		{
			#ifdef LINX_DEVICE_FAMILY=4 | LINX_DEVICE_FAMILY=6 //Both microcontrollers
			LinxDev->DelayMs(30);
			#endif
		}
	}
